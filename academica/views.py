...
def authcheck(request):
	...
	if user is not None:
		...
		return redirect('academica:clases')
	else:
		return redirect('inicio:sislogin')


def password_changed(request):
	template = 'academica/password_changed.html'
	context = {}
	return render(request, template, context)


def perfil_profesor(request):
	template = "academica/form_perfil_profesor.html"
	...
	if request.method == "POST":
		...
		if form.is_valid():
			form.save()
			return redirect('academica:perfil_profesor')
		else:
			return redirect('academica:clases')
	else:
...
@login_required
def clases(request):
	template = "academica/lista_clases.html"
...
def clase_periodos(request, clase_id):
	template = "academica/clase_periodos.html"
...
def editar_notas(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			return redirect('academica:clase_alumnos', clase.id, periodo.id)
...
def matricular_grupo(request):
	template = "academica/matricular_grupo.html"
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			template = "academica/matriculas_creadas.html"
...
def alumnos(request, tipo_reporte):
	template = "academica/alumnos.html"
...
def reporte_alumno_notas(request):
	template = "academica/alumno_notas.html"

	alumno = get_object_or_404(Alumno, pk=int(request.GET.get("cedula", 0)))
...
def reporte_quimestre(request):
	template = "academica/reporte_quimestral.html"
...
def reporte_matricula(request):
	template = "academica/reporte_matricula.html"
